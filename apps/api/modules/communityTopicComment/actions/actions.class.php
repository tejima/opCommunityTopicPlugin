...
class communityTopicCommentActions extends opJsonApiActions
{
  ...
  {
    if('' !== (string)$request['community_id']){
      $ct = Doctrine::getTable('CommunityTopic')->createQuery('ct')
        // ->where('ct.community_id = ?', $request['community_id'])
        ->where('ct.community_id = ?', (string)$request['community_id'])
        ->orderBy('ct.id ASC')
        ->limit(1)->fetchOne();
      if(!$ct->id){
        return $this->renderText(json_encode(array("status"=>"success","message"=>"empty")));    
      }
      $request['community_topic_id'] = (string)$ct->id;
    }
  ...
  public function executePost(sfWebRequest $request)
  {
    ...
    if('' !== (string)$request['community_id']){
      ...
      if(!$ct->id){
        ...
        $topic->member_id = 1; //FIXME
        $topic->name = $request['body'];
        $topic->body = $request['body'];
        $topic->save();
        $request['community_topic_id'] = (string)$topic->id;
      }
      $request['community_topic_id'] = (string)$ct->id;
    }
    $this->forward400If('' === (string)$request['community_topic_id'], 'community_topic_id parameter is not specified.');
